...
class Maybank {
  ...
  async click(name, selector) {
    await Promise.race([
      ...
      this.page.waitForSelector('#root > div > div > div.notifications-wrapper > div > div').then(() => {
        throw new Error(`Something went wrong: ${name}`);
      })
    ]);
  }
  ...
  async login(username, password) {
    try {
      ...
      await this.click("Login Button One", "#root > div > div > div.Header---container---kBsDt > div.col-md-12 > div > div > div > div:nth-child(2) > div > div > div > div > div:nth-child(3) > button").catch(e => console.log(e));
      ...
      await this.click("Modal Yes Button", "#root > div > div > div.Header---container---kBsDt > div:nth-child(2) > div > div > div > div > div:nth-child(2) > div.modal-footer > div > div.col-lg-6.col-md-6.col-sm-6.col-xs-12.SecurityPhrase---right-btn-container---32k8- > button").catch(e => console.log(e));

      let passwordInput = await this.page.waitForSelector("#badge", { visible: true });
      await passwordInput.type(password, { delay: 50 });

      await this.click("Login Button 2", "#root > div > div > div.Header---container---kBsDt > div:nth-child(2) > div > div > div > div > div:nth-child(2) > div.modal-footer > div > div.col-lg-5.col-md-5.col-sm-5.col-xs-12.SecurityPhrase---right-btn-container---32k8- > button").catch(e => console.log(e));

      await this.page.waitFor(500);
    } catch (e) {

      throw `Maybank Login: ${e.stack}`;
    }
  ...
  async transfer() {
    try {
      ...
      await this.click('Request TAC',
        '#scrollToTransactions > div.Transactions---container---3sqaa > div.Transactions---content---2P7lC > div.Transactions---withSide---2taIP.container-fluid.Transactions---summaryContainer---1rNvj.undefined > div > div > div.Transactions---stickyConfirmation---2aISx > div > div > div > div > div.col-md-8.col-xs-12 > div > div > div.col-sm-3.col-xs-12 > button').catch(e => console.log(e));;
    } catch (e) {
      throw `Maybank Transfer: ${e.stack}`;
    }
  }
  ...
  async resendTac() {
    try {
      ...
      throw `Maybank ResendTAC: ${e.stack}`;
    }
