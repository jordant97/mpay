...
class Public {
  ...
  async login(username, password) {
    ...
    let usernameInput = await frame.waitForSelector(
    ...
    await _puppeteer.click(frame, "Next Button", "#NextBtn");

    await _puppeteer.click(frame, "Yes Checkbox", "#passcred");
    await _puppeteer.click(frame, "Yes Checkbox", "#passcred");

    let passwordInput = await frame.waitForSelector("#password", {
      visible: true,
    });
    await passwordInput.type(password, { delay: 50 });

    await _puppeteer.click(frame, "Submit Button", "#SubmitBtn");
