...
class Public {
    ...
    async init() {
        ...
        try {
            ...
            await this.page.goto("https://www2.pbebank.com/myIBK/apppbb/servlet/BxxxServlet?RDOName=BxxxAuth&MethodName=login");
    ...
    async login(username, password) {
        let frame = this.page.frames()[2];
        let usernameInput = await frame.waitForSelector("#LoginId > table > tbody > tr:nth-child(3) > td > div > input.form-control.placeholder-no-fix");
        ...
        await _puppeteer.click(frame, 'Yes Checkbox', '#passcred');
        await _puppeteer.click(frame, 'Yes Checkbox', '#passcred');

        let passwordInput = await frame.waitForSelector("#password", { visible: true });
